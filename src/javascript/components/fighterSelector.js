...
function renderSelectedFighters(selectedFighters) {
  ...
  const previewElements = [firstPreview, versusBlock, secondPreview].filter((element) => !!element);

  fightersPreview.innerHTML = '';
  fightersPreview.append(...previewElements);
}
...
export function renderHealthBar(position, health, maxFighterHealth) {
  let healthBar = document.querySelector(`#${position}-fighter-indicator`);

  if (health <= 0) {
    healthBar.style.width = 0;
  } else {
    healthBar.style.width = `${Math.round((100 * health) / maxFighterHealth)}%`;
  }
}
